...
contract("LiquidityMiningStakingETH", async (accounts) => {
  ...
  before("setup", async () => {
    ...
    const _stakingMock = await LiquidityMiningStakingETH.new();
  ...
  describe("__LiquidityMiningStakingETH_init", () => {
    it("should revert if already was invoked", async () => {
      await truffleAssert.reverts(
        staking.__LiquidityMiningStakingETH_init(),
  ...
  describe("setDependencies", async () => {
    ...
    describe("should set dependencies correctly", async () => {
      ...
      it("stakingToken", async () => {
        expect(await staking.stakingToken()).to.be.eq(await contractsRegistry.getSushiswapBMIToETHPairContract());
      });
